...
def main():
    ...
    blogdata_path = REPO_ROOT / "client" / "src" / "content" / "blogData.ts"

    if not blogdata_path.exists():
        raise SystemExit(f"Could not find blogData at: {blogdata_path}")

    posts = load_posts_from_blogdata_js_like(blogdata_path)
